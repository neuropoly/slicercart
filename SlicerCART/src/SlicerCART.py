...
class SlicerCARTWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def onLinePlaced(self, caller, event):
        ...
        """
        lineNode = caller
        numPoints = lineNode.GetNumberOfControlPoints()
        if numPoints == 2:
            statuses = [lineNode.GetNthControlPointPositionStatus(i) for i in range(2)]
            if all(s == slicer.vtkMRMLMarkupsNode.PositionDefined for s in statuses):
                print("✅ Line fully placed!")
