...
class ShowSegmentVersionLegendWindow(qt.QWidget):
    def __init__(self, segmenter, segmentationInformation_df, parent=None):
        ...
        informativeLabel.setText(
            f'Legend of [{self.segmenter.labelOfCompareSegmentVersions}] '
            f'segment versions')
        ...
        if segmentationInformation_df.shape[0] > 0:
            ...
            self.versionTableView.setRowCount(
                len(
                    self.segmenter.
            ...
            for index, row in segmentationInformation_df.iterrows():
                ...
                for (segment_name, color) \
                    ...
                    if row['Segmentation version'] in segment_name:
                        ...
                        colorItem.setBackground(qt.QBrush(
                            qt.QColor(currentColor[0], currentColor[1],
                                      currentColor[2])))
                        self.versionTableView.setItem(index, 0, colorItem)
                        self.versionTableView.setHorizontalHeaderItem(
                            0, qt.QTableWidgetItem('Color'))
